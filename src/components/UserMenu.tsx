...
export function UserMenu({ userName, userRole }: { userName: string; userRole: string }) {
  ...
  return (
    <div ref={ref} className="relative">
      ...
      >
        <span className="flex h-8 w-8 items-center justify-center rounded-full bg-primary-100 text-xs font-semibold text-primary-700 dark:bg-primary/20 dark:text-primary-100">
          {initials(userName)}
        </span>
      ...
      {open && (
        <div className="absolute right-0 top-full z-20 mt-2 w-52 overflow-hidden rounded-md border border-ink-400/15 bg-white py-1 shadow-lg dark:border-white/10 dark:bg-ink-800">
          ...
          >
            <KeyIcon className="h-4 w-4 text-ink-400 dark:text-white/40" />
            Change password
          </Link>
          <button
            type="button"
